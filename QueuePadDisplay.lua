...
-- REMOTE EVENTS
-- These events are fired by the server whenever the queue state changes for a specific pad.
...
-- Centralized validation prevents duplicated nil checks and protects against runtime errors if pads are misconfigured.
...
-- Logic is client-side for responsiveness and to avoid unnecessary server-side UI replication.
...
QueueTimerEvent.OnClientEvent:Connect(function(pad: Model, timeLeft: number)
	...
	-- Timer visibility is derived from state instead of toggling the BillboardGui itself, preventing flicker or desync.
	if timeLeft > 0 then
